...
lengths = [3.144]#np.linspace(0.1,5,50)# [0.3,0.4,0.5,0.6,0.7,1,1.5,2,3,3.144,4,5]
fracs = np.linspace(0.05,0.7,50)
for L_shell_noflange in lengths:
    masses =[]
    dps =[]
    epsilons =[]
    ntus=[]
    for baffle_centre_spacing_frac in fracs:
        ...
        epsilons.append(eps)
        ntus.append(NTU)
        print(f"Length {L_shell_noflange:.1f}, spacing {baffle_centre_spacing_frac:.2f}, mass {m_HX/1e3:.1f}t, A_htc {A_htc:.2f} m2, U={U:.0f}W/m2/K,NTU={NTU:.1f},"\
    ...
    ls = '-' if L_shell_noflange==2 else '--'
    l2, =ax1.plot(fracs, dps, color='C1', label=rf'L={L_shell_noflange:.1f} m, $\Delta p/p_{{in}}$',ls=ls)
    #l1=ax2.scatter(lengths,ntus, marker='+',color='C0', label=r'NTU (-) ')
    l1,=ax2.plot(fracs,ntus, color='C0', label=rf'L={L_shell_noflange:.1f} m, $N_{{tu}}\propto UA$ (-)',ls=ls)
    #l3 = ax2.scatter(lengths, masses, marker='o', color='grey', facecolors='grey', label='Mass (t)')
    #l3, = ax2.plot(fracs, masses, color='grey', label=f'Mass (t), L={L_shell_noflange:.1f} m ',ls=ls)
    min_index = np.argmin(dps)
    max_index = np.argmax(ntus)
    ax1.scatter([fracs[min_index]],[dps[min_index]],color='C1')
    ax2.scatter([fracs[max_index]],[ntus[max_index]],color='C0')
    lines.extend([l1, l2])
...
# Set x-axis label
ax1.set_xlabel(r'Baffle separation fraction $L_{b,c}/L_{tube,eff}$')
fig.suptitle("Baffle spacing: Impact on performance")
fig.tight_layout()
plt.savefig('plots/baffle_spacing.svg', format='svg')
plt.show()
